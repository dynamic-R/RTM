...
RTMtutorial <- function(x = c("?", 
  ...
  if (x[1] == "?") {
    ...
    c("About the modelling course at Utrecht University",
      ...
      "Tips to apply when converting units"))
    return(tutorial)
  } else {
...
RTMexercise <- function(x = c("?",
    ...
    "dissolutionSi", "evaporation", "O18exchange",
    "equilibriumNH3", "equilibriumHCO3", "equilibriumOMD", 
    "detritus", "npzd",  "crops_weed", "covid", "virus", 
    "bears_salmon", "plant_coexistence", "hyacinth_algae", "aquaculture", 
    "estuaryAnoxia", "Pdiagenesis", "diagenesis", "pHprofiles"), 
...
RTMexerciseFULL <- function(x = c("?",
  ...
  subdirectory<- c("R", 
                  ...
                  "linearmodels", 
                  rep("chemistry", times=7),
                  rep("biogeochemistry", times=3),
                  rep("epidemiology", times=2),
                  rep("ecology", times=3),
                  "individualbased",
                  rep("reaction_transport", times=4)
                  )
