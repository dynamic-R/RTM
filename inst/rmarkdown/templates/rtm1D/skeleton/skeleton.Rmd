...
This template file contains a simple one-dimensional reaction-transport model describing the dynamics of molecular oxygen (O$_2$, in $mol~m^{-3}$) and biochemical oxygen demand (BOD, in $mol~m^{-3}$) in an estuary that is in contact with the air. 
...
With regard to the reaction, a simple first-order decay of BOD that is limited by oxygen is assumed. This reaction consumes both BOD and O2 with a stoichiometry of 1:1. O2 is also exchanged with the atmosphere.
...
pars <- c(      
  ...
  rDecay   = 0.05 , # [/d]        rate constant of BOD decay (first-order process)
  kO2      = 0.001, # [mol/m3]    half-saturation O2 concentration for BOD decay
  ...
  k        = 0.1    # [/d]        rate constant for air-water O2 exchange
)
...
BOD1D <-function(t, state, pars) {  # state is a long vector      
  ...
  # "extracted" from the LONG vector state passed to the function as input.
    O2  <- state[   1 :   N ]   # first N elements for O2
    BOD <- state[(N+1):(2*N)]   # second N elements for BOD
...
p2["k"] <- 0            # set reaeration rate constant to 0
...
```{r, fig.height = 4, fig.width = 8}
plot(std, std2, grid = Grid$x.mid,  # plot state variables
...
```{r}
toselect <- c("TotalDecay", "TotalAeration", 
              "O2influx", "O2efflux", "BODinflux", "BODefflux")
BUDGET   <- std[toselect]
unlist(BUDGET)
