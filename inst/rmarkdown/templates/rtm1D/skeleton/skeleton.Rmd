...
This template file contains a simple one-dimensional reaction-transport model describing the dynamics of molecular oxygen (O$_2$, in $mol~m^{-3}$) and biochemical oxygen demand (BOD, in $mol~m^{-3}$) in an estuary that is in contact with air. 
...
With regard to the reactions, a simple first-order decay of BOD that is limited by O2 is assumed. This reaction consumes both BOD and O2 with a stoichiometry of 1:1. O2 is additionally exchanged with the atmosphere.
...
* O2: imposed concentration upstream (*C.up*), zero-gradient downstream (default, need not be specified).
...
BOD1D <-function(t, state, pars) {  # state is a long vector      
  ...
  list(c(dO2.dt, dBOD.dt),  # time-derivatives 
