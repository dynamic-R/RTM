...
The model is rewritten in terms of lump sum species $Btot = [AB] + [B]$ and $Atot = [AB] + [A]$. For more information on the model, see the reader "Local Equilibrium Chemistry in R --- part I: One Equilibrium Reaction". This reader can be viewed using the R-command ``RTMexercise("equilibriumNH3")``.
...
# model parameters
parms <- c (kf      = 1000,    # [/s]         forward rate constant
            kb      = 2e6,     # [/mol m3 /s] backward rate constant
            lambda  = 0.1      # [/s]         rate constant for the first-order removal of A
                               # note that lambda << kf and lambda << kb*Btot
                               # (i.e., the local equilibrium assumption is valid)
)
```

A function to numerically solve for the concentration of species $B$ based on the concentration of species $Atot$ and $Btot$ (assuming local equilibrium).
