...
* O$_2$ is added by air-water exchange ($k_{Aeration}=0.1~d^{-1}$, O$_{2,sat} = 0.3~mol~m^{-3}$),
...
BODmodel <-function(t, state, pars) { 
  ...
  # return time-derivatives and ordinary variables as a list     
    list(c(dO2.dt, dBOD.dt), # vector with derivatives 
...
outtimes <- seq(from = 1, to = 100, length.out = 100) # vector of output times
...
pars2["kAeration"] <- pars["kAeration"]/10 # 10-fold lower kAeration
...
```{r, message=FALSE}
std <- runsteady(y = yini, parms = pars, func = BODmodel)
std$y
```
...
plot(k.seq, O2.seq,  type="l", xlab="kAeration [/d]", main="O2",  ylab="mol/m3")
plot(k.seq, BOD.seq, type="l", xlab="kAeration [/d]", main="BOD", ylab="mol/m3")
```
