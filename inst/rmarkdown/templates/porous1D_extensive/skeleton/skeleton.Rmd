...
The biogeochemical process is the first-order mineralization of POC to DIC, with a stoichiometry of 1:1 (i.e., POC -> DIC).
...
Diamodel <- function (t, state, pars)   # state is a LONG vector
{
  with (as.list(pars),{ 
    ...
    # === reaction rates ===
    # POC mineralisation
    Mineralisation <- rMin * POC   # [molPOC/m3 SOLID/d] (per volume of solid!)
    ...
    dPOC.dt   <- ( tran.POC$dC                              # transport
                 - Mineralisation )                         # reactions, [molPOC/m3 SOLID/d] !!
    
    # dissolved substances
    poro      <- porLiquid$mid
    dDIC.dt   <- ( tran.DIC$dC +                            # transport
                   POC2DIC*Mineralisation * (1-poro)/poro ) # reactions, [molDIC/m3 LIQUID/d] !!
...
c( BUDGET$POC.Deep.Flux - BUDGET$POC.SWI.Flux, 
  -BUDGET$TotalMin )
# net DIC rate is positive => DIC is net produced within the domain
c( BUDGET$DIC.Deep.Flux - BUDGET$DIC.SWI.Flux,  
   BUDGET$TotalMin )
```
...
Based on that, we can plot the profile of POC at a certain time or the times-series of POC at a particular depth.
