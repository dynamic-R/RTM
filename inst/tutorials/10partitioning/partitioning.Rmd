...
msg <- sprintf("The answer is $k_d/k_p$. Tip: if you do not remember whether this would be $k_p/k_d$ or $k_d/k_p$, think about the units. Concentrations are in $mol~m^{-3}$, and this unit is obtained with $k_d/k_p$, not with $k_p/k_d$. (there are two correct answers in the list)")
...
* The volume and surface area of a sphere with radius $r$ is given by: $4/3 \times \pi \times r^3$ and $4 \times \pi r^2$.
...
```{r dSidt-solution}
Ceq    <- kd/kp*1000                # equilibrium concentration (umol/L=mmol/m3)
A      <- 4*pi*(Dini/2*1e-6)^2 * N  # initial total active surface (m2)
A.V    <- A/(V*1e-3)                # area density (total surface area per volume) (/m)
dSi.dt <- -kp*A.V*(Cini - Ceq)      # the change in time umol/L/yr
```
...
question(sprintf("A well-mixed anoxic water column is in contact with the atmosphere. What is the initial rate of increase in the $O_2$ concentration in the water? Piston velocity, $k_p$, is $1~m~d^{-1}$, $O_2$ solubility in the water is $300~\\mu mol~L^{-1}$, water column height, h, is $50~cm$."), type = "learnr_radio",
