...
- r \cdot C
$$
where $C$ is in units of $mol~C~m^{-3}_S$, $x$ is sediment depth in $m$, $t$ is time in $days$, $\phi_x$ is the sediment porosity, $D_b$ is the bioturbation mixing coefficient, $v$ is the advective velocity (in $m~day^{-1}$; equal to the sediment accretion velocity), and $r$ is the rate constant describing POC mineralization (mineralization is assumed to be described by first-order kinetics).

The boundary conditions are as follows:

* POC flux at the sediment-water interface is $1~mmol~C~m^{-2}~d^{-1}$, i.e., $(1-\phi_x) D_b \frac{ \partial C}{\partial x}= 1\times 10^{-3}~mol~m^{-2}~d^{-1}$.
...
* Assume that the effective sediment diffusion coefficient of DIC is $D_{DIC} = 5\times 10^{-5}~m^2~d^{-1}$ and the bottom water DIC concentration is $0.002~mol~m^{-3}$. Add these two parameters to the parameter vector.
