...
## Challenging questions - response time and equilibrium
...
Quantify the *response time* of the system ($\tau_{AB}$), and the amounts of carbon in both reservoirs in this new equilibrium ($A_{eq}$ and $B_{eq}$). Use R and the values of X, Y and Z defined earlier in this tutorial to calculate $\tau_{AB}$, $A_{eq}$ and $B_{eq}$. Use the R-code from the earlier part of this tutorial to verify that your calculated values agree with those predicted by the numerical model.
...
<div id="Tab-hint">
**Hint:** To solve this exercise, use first the 'pen & paper method' and do a little bit of maths.
<BR> 1. Write the mass balance equations for $A$ and $B$. Substitute $$A=A_{eq} + \Delta A\cdot e^{-r\cdot t}$$ as a solution for $A$, and $$B = B_{eq} + \Delta B\cdot e^{-r\cdot t}$$ as a solution for $B$, where $r$ is a parameter describing the rate of decay in the carbon content in both reservoirs. Verify that these functions are, indeed, solutions to the mass balance equations for $A$ and $B$.
<BR> 2. If you assume that $t$ is a very large number ($t\gg 1/r$), i.e., corresponding to time when the system reaches an equilibrium, you will be able to derive the following relationship: $$r_{AB}\cdot A_{eq} = r_{BA}\cdot B_{eq}$$. 
...
<BR> 4. By simplifying the equation obtained in step 1, and combining it with relationships obtained in steps 2 and 3, you will be able to derive a relationship for $r$. The *response time* is then calculated, by definition, as $$\tau_{AB}=1/r.$$
...
<BR> 6. By combining the relationships obtained in steps 2 and 5, you will be able to derive formulas for $A_{eq}$ and $B_{eq}$. As mentioned above, most of this exercise should be solved using the 'pen & paper method'.
</div>
...
To apply the knowledge you gained so far, expand the 2-box model from this tutorial towards a more complicated system: the Earth's global carbon cycle.

Type the following commands in the R console to get the full description of this exercise.
...
The global carbon cycle model is a wonderful example of models that study the response of perturbed systems. If you are curious and would like to learn more about perturbed systems, and specifically about how to find the time scales on which they respond --- the so-called *response time* --- you can do so by studying the following extra readers.
