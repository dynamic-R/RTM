...
```{r enzUse, echo=FALSE}
msg <- "When [A] is very small ( $ [A] << K_A $ ), the Michaelis-Menten rate expression can be approximated according to $\\frac{[A]}{[A]+K_A}\\approx \\frac{[A]}{K_A} = \\frac{1}{K_A}\\cdot [A]$, which is first-order in [A]."
...
msg <- "When [A] is very large ($[A] >> K_A$), the Michaelis-Menten rate expression can be approximated as $\\frac{[A]}{[A]+K_A} \\approx \\frac{[A]}{[A]} \\equiv 1$, which is zero-order in [A]."
