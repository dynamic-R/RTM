...
```{r enzUse, echo=FALSE}
msg <- "When [A] is very small, the Michaelis-Menten rate expression $\\frac{[A]}{[A]+K_A}$ can be approximated as: $\\frac{[A]}{K_A}$ ~ $r[A]$ which is first-order in [A]"
...
msg <- "When [A] is very large, the Michaelis-Menten rate expression $\\frac{[A]}{[A]+K_A}$ can be approximated as: $\\frac{[A]}{[A]} \\approx 1$ which is zero-order in [A]"
...
ks <- 10
par(mfrow=c(1,3))
plot(x=A,y=A/(A+10), main="Michaelis-Ment3n", ylab="", xlab="[A]", type="l", lwd=2)
plot(x=A,y=A/ks, main="first-order", ylab="", xlab="[A]", type="l", lwd=2)
plot(x=A,y=rep(1, times=101), main="zero-order", ylab="", xlab="[A]", type="l", lwd=2)
```

## Recognizing rate-limitation

```{r enzRateLim, echo=FALSE}
msg <- sprintf("The dependency of the process on A is given by the Michaelis-Menten kinetics $\\frac{[A]}{[A]+K_A}$; the dependency on B is given by $\\frac{K_B}{[B] + K_B}$ which is equal to $1-\\frac{[B]}{[B] + K_B}$, and this will decrease with increasing [B] (inhibition); the dependency on C is given by $r \\times [C]$, which is a first-order function with respect to [C]")
