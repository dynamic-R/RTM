...
# Model function
Scheldt1D <- function(t, state, pars) {   # state is a LONG vector of state variables
  with(as.list(pars),{
    ...
    return(list(c(dOxygen.dt, dNitrate.dt, dAmmonia.dt),   # the rates of change
           ...
           # total fluxes in+out at model boundaries
           TotNinflux     = TranAmmonia$flux.up + TranNitrate$flux.up,
           TotNefflux     = TranAmmonia$flux.down + TranNitrate$flux.down,
           # integrated rates along the modeled domain
           TotalNitrification = sum(Nitrification * Grid$dx),
...
legend("topright", legend = c("current", "1970s"), lty=1, lwd=2, col=1:2, bty="n")
```
