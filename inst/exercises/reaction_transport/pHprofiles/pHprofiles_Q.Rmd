...
date: "April 2021, update: March 2026"
output:
  pdf_document:
    toc: false
  html_document:
...
In two other readers, we have shown an implementation of two models in R: (i) a box model that can predict pH variation over time due to reactions that add/remove chemical species capable of accepting/donating protons (i.e., acid-base reactions), and (ii) a one-dimensional reaction-transport model that can predict the variation of ammonia, nitrate and oxygen along an estuary. Here we combine and expand these two models to illustrate how to implement in R a reaction-transport model that can predict the \textbf{spatial variation of pH}. The overall approach involves two steps. First, the \texttt{ReacTran} package is used to calculate the steady-state distribution of the lump-sum species that include total ammonia ($\Sigma NH_4$), total nitrate ($\Sigma HNO_3$), total dissolved inorganic carbon ($\Sigma CO_2$), and (titration) proton alkalinity ($TA$). Then, based on these profiles, the \texttt{AquaEnv} package is used to calculate the steady-state distribution of pH.
...
It is important that you have fully understood the material covered in those readers before you start reading the following text. Type the following commands in the R-console to view the readers:
...
out <- 1e3*data.frame(
  TA    = AE$TA,                                                # contributions due to:
  CO3   = ifelse(length(AE$CO3 )==1, rep(AE$CO3, 2), AE$CO3),   # carbonate 
  HCO3  = ifelse(length(AE$HCO3)==1, rep(AE$HCO3,2), AE$HCO3),  # bicarbonate 
  H2SO4 = AE$H2SO4, HSO4 = AE$HSO4, SO4  = AE$SO4,              # sulphate
  BOH4  = AE$BOH4,                                              # borate
...
require(knitr)
kable(out,  
      caption = "Concentrations of selected ions in seawater (in mmol/kg liquid).")
```
...
plot(x=AE$TA, y=AE$pH, xlab="Alkalinity [mol/kg Liquid]", ylab="pH", 
     main="aquaenv output", las=1, lwd=2, type="l")
```
...
Grid    <- setup.grid.1D(L = Length, N = N, dx.1 = 10)
