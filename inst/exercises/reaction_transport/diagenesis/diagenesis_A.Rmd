...
$$\begin{array}{l}
R_{Aero} = r_{Min} \cdot \frac{O_2}{O_2+k_{O_2}} \cdot Corg \\[5mm]
R_{Nitri} = r_{Nit} \cdot \frac{O_2}{O_2+k_{O_2}} \cdot NH_3 \\[5mm]
R_{Denitri} = r_{Min} \cdot \frac{k_{O_2}}{O_2+k_{O_2}} \cdot \frac{HNO_3}{HNO_3+k_{HNO_3}} \cdot Corg \\[5mm]
R_{Sred} = r_{Min} \cdot \frac{k_{O_2}}{O_2+k_{O_2}} \cdot \frac{k_{HNO_3}}{HNO_3+k_{HNO_3}} \cdot \frac{H_2SO_4}{H_2SO_4 + k_{H_2SO_4}} \cdot Corg \\[5mm]
R_{Sox} = r_{Sox} \cdot \frac{O_2}{O_2+k_{O_2}} \cdot H_2S \\[5mm]
R_{Methanogen} = r_{Min} \cdot \frac{k_{O_2}}{O_2+k_{O_2}} \cdot \frac{k_{HNO_3}}{HNO_3+k_{HNO_3}} \cdot \frac{k_{H_2SO_4}}{H_2SO_4+k_{H_2SO_4}} \cdot Corg
\end{array}
$$

Using subscripts $S$ and $L$ to refer to the solid and liquid phase of the sediment, respectively, units of the rates are:
  
  * $R_{Aero}$, $R_{Denitri}$, $R_{Sred}$ and $R_{Methanogen}$ are in $mol~C~m^{-3}_S~d^{-1}$.
  
  * $R_{Nitri}$ is in $mol~N~m^{-3}_L~d^{-1}$.
  
  * $R_{Sox}$ is in $mol~S~m^{-3}_L~d^{-1}$.

Note that $R_{Aero}+R_{Denitri}+R_{Sred}+R_{Methanogen}$ is equal to $r_{Min} \cdot Corg$, as follows directly from the fact that the sum of the limitation and inhibition terms for a specific substrate is 1. Thus, the *total* mineralisation rate is first-order with respect to $Corg$ and independent of the terminal electron acceptor. The limitation and inhibition terms only dictate the *contributions* of the different oxidants to the overall mineralisation process.

## Mass balance equations

The updated mass balance equations that include all processes mentioned are:
$$
\begin{array}{rcl}
...
\\[6mm]
& &- R_{Aero} - R_{Denitri} - R_{Sred} - R_{Methanogen} \\[6mm]
...
\\[6mm]
& & - R_{Aero} \cdot f_{2L} - 2 \cdot R_{Nitri} - 2 \cdot R_{Sox} \\[6mm]
...
\\[6mm]
& & + \frac{16}{106} \cdot  (R_{Aero} + R_{Denitri} + R_{Sred} + R_{Methanogen}) \cdot  f_{2L} - R_{Nitri} \\[6mm]
\frac{\partial HNO_3}{\partial t} & = & \frac{1}{\phi} \cdot \frac{\partial}{\partial x}\left[\phi\cdot  D_{HNO_3}\cdot  \frac{ \partial HNO_3}{\partial x}\right] 
-\frac{1}{\phi}\cdot \frac{ \partial }{\partial x}\left[\phi\cdot v\cdot HNO_3\right]
\\[6mm]
& & - 0.8 \cdot R_{Denitri} \cdot f_{2L} + R_{Nitri} \\[6mm]
\frac{\partial H_2SO_4}{\partial t} & = & \frac{1}{\phi}\cdot \frac{\partial}{\partial x}\left[\phi\cdot D_{H_2SO_4}\cdot \frac{ \partial H_2SO_4}{\partial x}\right] 
-\frac{1}{\phi}\cdot \frac{ \partial }{\partial x}\left[\phi\cdot v\cdot H_2SO_4\right] 
\\[6mm]
& & - 0.5 \cdot R_{Sred} \cdot f_{2L} + R_{Sox} \\[6mm]
...
\\[6mm]
& & + (R_{Aero} + R_{Denitri} + R_{Sred} + 0.5\cdot R_{Methanogen})\cdot f_{2L} \\[6mm]
...
\\[6mm]
& & + 0.5\cdot R_{Methanogen}\cdot f_{2L}
\end{array}
$$
where $f_{2L}=(1-\phi)/\phi$. **Note** the presence of factors $\phi$ or $1-\phi$ in the equations. These factors *cannot* be canceled out from the spatial derivative because, in general, the porosity $\phi$ varies with depth. Also note how each mass balance equation comprises two key terms: one accounting for the effects of *transport* (by diffusion/dispersion and advection; first line), and the other accounting for the effects of *reactions* (second line).
...
porFun.L  <- function(x, por.SWI, por.deep, k_phi)
  return( por.deep + (por.SWI-por.deep)*exp(-x*k_phi) )

# function describing the SOLID volume fraction (svf = 1-porosity)
porFun.S  <- function(x, por.SWI, por.deep, k_phi)
  return( 1-porFun.L(x, por.SWI, por.deep, k_phi) )

# calculate porosity and svf on the grid (mid-points and box interfaces, etc.)
porLiquid <- setup.prop.1D(func=porFun.L, grid=Grid, por.SWI=0.9, por.deep=0.7, k_phi=100)
porSolid  <- setup.prop.1D(func=porFun.S, grid=Grid, por.SWI=0.9, por.deep=0.7, k_phi=100)
