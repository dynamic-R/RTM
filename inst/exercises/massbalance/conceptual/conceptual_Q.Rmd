...
date: "June 2021, updated February 2026"
output:
...
# Exercise 5.	Phosphorus cycling in marine sediments

The dynamics of phosphorus ($P$) in sediments is complex because it is impacted by biogeochemical and physical processes. Examples of relevant processes include mineralization of organic matter, which leads to phosphate release, precipitation as apatite (calcium phosphate mineral) or adsorption to calcium carbonate (together denoted as $Ca_xP_y$), or adsorption to iron oxides. Adsorption to iron oxides occurs in the oxic zone, while phosphate is released again when iron oxides are reduced in the anoxic zone of the sediment. In the anoxic zone, organic matter mineralization can occur using iron oxides as the electron acceptor, leading to the production of reduced iron (and dissolved phosphate from the mineralized organic matter but also from the reduction of iron oxides containing the adsorbed phosphate), whereas reduced iron can be transformed back to oxidized iron in the oxic zone. Authigenic apatite formation and $P$ adsorption to $CaCO_3$ can play an important role in the long-term removal of $P$, while iron oxides can play an important role in preventing the leakage of phosphate from sediment into the overlying water.

## Tasks

Use the above information to create a conceptual diagram for studying $P$ dynamics in marine sediments. 

* Start with a simpler model, ignoring the role of iron cycling on $P$.

* When you are done, construct a more realistic model that does include iron cycling. Carefully consider which new state variables, and which sediment zones, you will need to include in this model.

* In both models, additionally consider sedimentation, solute exchange across the sediment-water interface (e.g., by diffusion or bioirrigation), and burial due to sediment accretion, as the relevant transport-related input and output fluxes across the sediment column boundaries. Carefully consider which transport fluxes affect solutes and solids.