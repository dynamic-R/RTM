...
- AeroMin - Denitri - Sred - CH4gen \\[4mm]
...
+ \frac{16}{106} \cdot  (AeroMin + Denitri + Sred + CH4gen) \cdot  f_{2L} - Nitri \\[4mm]
...
+ (OxicMin + Denitri + Sred + 0.5\cdot CH4gen)\cdot f_{2L} \\[4mm]
...
state    <- c(POC.ini, CO2.ini, O2.ini, NH3.ini, NO3.ini, SO4.ini, H2S.ini, CH4.ini)
```
...
Diamodel <- function (t, state, pars)   # state is a LONG vector
{
  with (as.list(pars),{ 
    ...
    CO2 <- state[  (N+1):(2*N)]    # next N elements:  CO2
    ...
    CH4genesis   <- rMin * kO2/(O2+kO2) * kNO3/(NO3+kNO3) * kSO4/(SO4+kSO4) * POC
    
    Mineralisation  <- AeroMin  + Denitri + SO4reduction + CH4genesis
    ...
    dPOC.dt   <- ( tran.POC$dC                                    # transport
                 - AeroMin - Denitri - SO4reduction - CH4genesis) # reactions, 
    ...
    dCO2.dt   <- ( tran.CO2$dC
                   + (AeroMin + Denitri + SO4reduction + 0.5*CH4genesis)*f2Liquid )
    dO2.dt    <- ( tran.O2$dC - 2*Nitri - 2*Sox - AeroMin*f2Liquid )
    dNH3.dt   <- ( tran.NH3$dC - Nitri 
                   + 16/106*(AeroMin + Denitri + SO4reduction + CH4genesis)*f2Liquid )  
    ...
    dCH4.dt   <- ( tran.CH4$dC + 0.5*CH4genesis*f2Liquid)
    ...
    TotalCH4gen <- sum(CH4genesis    *Grid$dx*porSolid$mid)
...
```{r, fig.width=8, fig.height=8}
plot(std, std1, std2, 
 which = c("AeroMin","Denitri","SO4reduction","CH4genesis","Mineralisation","Nitri","Sox"),
...
par(mfrow=c(1,3))
s <- std
matplot(y=Grid$x.mid, x=cbind(s$AeroMin, s$Denitri, s$SO4reduction, s$CH4genesis), 
...
matplot(y=Grid$x.mid, x=cbind(s$AeroMin, s$Denitri, s$SO4reduction, s$CH4genesis), 
        type="l", ylim=c(Length,0), col=1:4, lwd=2, lty=1,
        xlab="molC/m3 Solid/d", ylab="depth (m)")
legend("bottomright",legend=c("AeroMin", "Denitri", "SO4reduction", "Ch4genesis"),
