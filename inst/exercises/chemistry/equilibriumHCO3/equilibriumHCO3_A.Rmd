...
\qquad\hspace{2cm} (3c)
$$
Here, we defined the dissociation constants according to $K_1 = k_{1f}/k_{1b}$, $K_2 = k_{2f}/k_{2b}$, and $K_w' = k_{3f}/k_{3b}$. The constant $K_w'$ is typically expressed in terms of the ionic product of water, $K_w = [OH^-]\cdot [H^+]$. A comparison of this definition with Eq. 3c yields a relationship $K_{w} = K_w' \cdot [H_2O]$.
If relationships 3a--c are true, then the time derivatives in equations 1b--f are zero. This means that these three equations do *not* provide any *extra* information for the equilibrium concentrations.
...
$$
Finally, by summing equation 1d with equation 1f and additionally considering the proposed definition of the lump-sum variable $[H_2O_{tot}]$, we obtain
$$
\frac{d[H_2O_{tot}]}{dt} = 0. \qquad\hspace{2cm} (4c)
$$

### Key take-home message

These equations show that under the assumption of a local chemical equilibrium, $DIC$ is *added* at a rate of the *slow process* ($R_C$), while $ALK$ and $H_2O_{tot}$ are *conserved* (not changing over time). Notably, $DIC$, $ALK$ and $H_2O_{tot}$ are *not* affected by the fast reversible reactions!
...
Regarding the concentration of $[OH^-]$, it can be calculated from $[H^+]$ based on the relationship 3c. First, by substituting $[H_2O]$ based on the definition of $H_2O_{tot}$, we obtain
$$
K_w = \frac{[OH^-]\cdot [H^+]}{[H_2O_{tot}] - [OH^-]},
$$
which upon rearrangement yields the accurate relationship
$$
[OH^-] = \frac{K_w'}{K_w' + [H^+]}\cdot [H_2O_{tot}]. \qquad\hspace{2cm} (5d)
$$
This relationship can be simplified considering that for typical pH values of 1--12, the concentrations of $H^+$ are in the range of $10^{-12}$--$10^{-1}~mol~L^{-1}$. Thus, the dissociation constant $K_w' = 1.81\times10^{-16}~mol~L^{-1}$ in the denominator can be neglected, yielding 
$$
[OH^-] \approx \frac{K_w'}{[H^+]}\cdot [H_2O_{tot}].
$$
Using the same argument, the ratio $K_w'/[H^+]$ is in the range of $10^{-15}$--$10^{-4}$, which makes the concentration of $[OH^-]$ negligible in comparison to the concentration of $H_2O$ (i.e., $[H_2O_{tot}]\approx [H_2O]$). Thus, using the last equation, the expression for $[OH^-]$ can be well approximated by\footnote{This result is equivalent to the commonly used expression of the water selfionization equilibrium $K_w = [OH^-]\cdot [H^+]$.}
$$
[OH^-] = \frac{K_w}{[H^+]}. \qquad\hspace{2cm} (5d')
$$

Once the concentrations $[HCO_3^-]$, $[CO_3^{2-}]$, $[OH^-]$, and $[H^+]$ are known, the carbonate alkalinity is calculated based on the definition:
...
Another important point to note here is the statement 3: $[H^+]$ *can be calculated* if the values of $[DIC]$ and $[ALK]$ are known. This statement is the basis for the *general* approach in pH modeling: one needs to first *identify the appropriate lump-sum species* (e.g., $DIC$ and $ALK$) and then formulate the *effect of each slow process* on these lump-sum species (e.g., addition or removal of $DIC$ [e.g., 4a] and $ALK$ [e.g., 4b]).\footnote{Strictly speaking, one should also identify the effects of the slow processes on the lump-sum species $H_2O_{tot} = H_2O + OH^-$. However, due to the vastly larger concentration of $H_2O$ in an aquatic setting compared to all other species involved, the effects of processes such as mineral dissolution/precipitation, redox or acid-base reactions, and others, are typically negligible and the water concentration can be considered as constant.} Then, by solving the differential equations for the lump-sum species numerically, one can quantify the corresponding evolution of pH based on the statement 3. This will be illustrated in the following section. 
...
```{r}
solveALK <- function(K1 = 0.773e-06, K2 = 0.352e-09, Kw = 1e-14, DIC, pH){
...
Second, we define a function that calculates $[H^+]$ from the known carbonate alkalinity and DIC concentration (equation 5e). This is done by finding, using a numerical solver, the root of a function defined as the left-hand-side minus the right-hand-side of equation 5e.

```{r}
solveH <- function(K1 = 0.773e-06, K2 = 0.352e-09, Kw = 1e-14, DIC, ALK){
...
It is always good to test the functions. For $[DIC] = 2~mmol~L^{-1}$ and $[ALK] = 2.2~mmol~L^{-1}$, we expect $pH$ to be around $8$, and when we substitute that pH value to calculate $[ALK]$, we expect to get the original value. Indeed, the results are as expected:

```{r}
(pH.new  <- -log10(solveH(DIC = 2.0e-3, ALK = 2.2e-3)))
(ALK <- solveALK(DIC = 2e-3, pH = pH.new))
```

On the other hand, if the water contains no dissolved $CO_2$ ($[DIC] = 0$) and has zero carbonate alkalinity ($[ALK]=0$), we expect its pH to be 7. Indeed, that is what we obtain:
...
pars    <- c(RC = 0.1e-3,    # [mol/L/day]
             K1 = 0.773e-06, #K1 = 1.422e-06, # [mol/L]
             K2 = 0.352e-09, #K2 = 1.082e-09, # [mol/L]
             Kw = 1e-14)     # [mol^2/L^2]
...
We see that the addition of $CO_2$ at the given rate leads to an acidification of the water from the initial pH of 8 to pH of about 6.5 after 10 days.
