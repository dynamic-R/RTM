...
You should arrive at *three* equations that relate the equilibrium concentrations of [$H_2CO_3$], [$HCO_3^-$], [$CO_3^{2-}$], [$H_2O$], [$OH^-$], and [$H^+$]. To simplify the relationships, define the ratios between the forward and backward rate constants as the so-called dissociation constants: $K_1 = k_{1f}/k_{1b}$, $K_2 = k_{2f}/k_{2b}$, and $K_w' = k_{3f}/k_{3b}$. For a temperature of $25\,^\circ$C, salinity 0 and pressure of 1 bar, their values are approximately $K_1 = 0.773\times10^{-6}~mol~L^{-1}$, $K_2=0.352\times10^{-9}~mol~L^{-1}$, and $K_w' = 1.81\times10^{-16}~mol~L^{-1}$, where the latter value is calculated according to $K_w' = K_w/[H_2O]$ using $K_w= [OH^-]\cdot [H^+] = 1\times 10^{-14}~mol^2~L^{-2}$ for the \emph{ionic product of water} (Hawkes, 1995) and $[H_2O] \approx 55.3~mol~L^{-1}$ for the concentration of water.\footnote{The water concentration is derived from the density ($997~kg~m^{-3}$) and molar weight ($18.0153~g~mol^{-1}$) of pure water at $25\,^\circ$C.} The values for these constants at other conditions can be calculated using R-packages *seacarb* (Gattuso et al., 2020) and *AquaEnv* (Hofmann et al., 2010). 
