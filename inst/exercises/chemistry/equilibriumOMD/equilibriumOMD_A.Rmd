...
Here, we defined the dissociation constants $K_1$, $K_2$, $K_n$, $K_w'$ according to $K_1 = k_{1f}/k_{1b}$, $K_2 = k_{2f}/k_{2b}$, $K_n = k_{3f}/k_{3b}$, and $K_w' = k_{4f}/k_{4b}$. If these relationships are true, then the time derivatives in equations 1a--h are zero. This means that these four equations do *not* provide any *extra* information for the equilibrium concentrations.

## Task 3: Lump-sum species

Here we combine equations 1a--h and see what they imply for the suggested lump-sum species. By summing equations 1a, 1b and 1c, we obtain 
...
$$
These equations show that DIC is *added* at a rate of the *slow process*, $R_C$, $NH_x$ is *added* at a rate of the *slow process*, $R_N$, while $H_xO$ and ALK are *conserved* (not changing over time). Specifically, *none* of the lump-sum species DIC, $NH_x$, $H_xO$, and ALK is affected by the fast reversible reactions.
...
$$
[HCO_3^-]= \frac{K_{1}[H^+]}{[H^+][H^+] + K_{1}[H^+] +  K_{1} K_{2}}\cdot [DIC]
\qquad\hspace{2cm} (5a)
$$

$$
[H_2CO_3] = \frac{[H^+][H^+]}{[H^+][H^+] + K_{1}[H^+] +  K_{1} K_{2}}\cdot [DIC]
\qquad\hspace{2cm} (5b)
$$

$$
[CO_3^{2-}] = \frac{K_1 K_2}{[H^+][H^+] + K_{1}[H^+] +  K_{1} K_{2}}\cdot [DIC]
\qquad\hspace{2cm} (5c)
$$

$$
[NH_3] = \frac{K_n}{K_n + [H^+]}\cdot [NH_x] \qquad\hspace{2cm} (5d)
$$

$$
[NH_4^+] = \frac{[H^+]}{K_n + [H^+]} \cdot [NH_x] \qquad\hspace{2cm} (5e)
$$

$$
[OH^-] = \frac{K_w'}{K_w' + [H^+]}\cdot [H_xO] \approx \frac{K_w}{[H^+]} \qquad\hspace{2cm} (5f)
$$
Note that we provide both the accurate and approximate relationship between $[OH^-]$ and $[H^+]$ (see notes on this topic in Part II of the Reader). The accurate relationship will be relevant when evaluating the model in the limit $H_xO \rightarrow 0$, as requested in Task 6.

Once the concentrations $[HCO_3^-]$, $[CO_3^{2-}]$, $[NH_4^+]$, $[OH^-]$, and $[H^+]$ are known, the alkalinity can be calculated from the definition,
$$
[ALK] = [HCO_3^-] + 2\cdot [CO_3^{2-}] + [OH^-] - [NH_4^+] - [H^+]
\qquad\hspace{2cm} (5g)
$$
Because $[HCO_3^-]$ and $[CO_3^{2-}]$ depend on $[DIC]$ and $[H^+]$ (Eqs. 5a and 5c), $[NH_4^+]$ depends on $[NH_x]$ and $[H^+]$ (Eq. 5d), and $[OH^-]$ depends on $[H^+]$ (Eq. 5f), \bf equation 5g provides an intimate relationship between $[ALK]$, $[DIC]$, $[NH_x]$, and $[H^+]$ \rm when the system is in an equilibrium. Using this relationship, one can calculate any of the species provided that the other \emph{three} species are known. Because equation 5g is highly non-linear, these calculations are done numerically.

## Task 5: Application to organic matter mineralization

### R implementation

First, we expand the function that calculates alkalinity by including the concentration of $NH_4^+$ (Eq. $5g$). Additionally, in contrast to the approach in Part II, we will now use the accurate expression for calculating $[OH^-]$ from $[H^+]$ and $[H_xO]$ (Eq. $5f$). Thus, we include the concentration of $H_xO$ as an additional input to the function and the equilibrium constant $K_w'$ instead of $K_w$.

```{r}
solveALK <- function(K1=4.44e-07, K2=4.67e-11, Kn=5.68e-10, Kw.prime=1.79e-16,
                     DIC=0, NHx=0, HxO=55.3, pH=7){
  
  H      <- 10^(-pH)
  HCO3   <- K1*H /  (H^2 + K1*H + K1*K2) * DIC    # eq 5a
  CO3    <- K1*K2 / (H^2 + K1*H + K1*K2) * DIC    # eq 5c
  NH4    <- H / (H + Kn) * NHx                    # eq 5e
  OH     <- Kw.prime / (Kw.prime + H) * HxO       # eq 5f, accurate
  ALK    <- HCO3 + 2*CO3 + OH - NH4 - H           # eq 5g
  
  return(ALK)
}
```

Next, we update the function that calculates $[H^+]$ from given concentrations of $ALK$, $DIC$, $NH_x$, and $H_xO$ (Eq. $5g$). This is done by finding a root of a function defined as the left-hand-side minus the right-hand-side of Eq. $5g$.

```{r}
solveH <- function(K1=4.44e-07, K2=4.67e-11, Kn=5.68e-10, Kw.prime=1.79e-16,
                     DIC=0, NHx=0, HxO=55.3, ALK=0){
  
  # function whose root has to be sought
  rootFun <- function(H) {
    ALK.est <- solveALK(K1=K1, K2=K2, Kn=Kn, Kw.prime=Kw.prime,
                        DIC=DIC, NHx=NHx, HxO=HxO, pH=-log10(H))
    return(ALK.est - ALK)
  }
...
The function that implements the model based on the local equilibrium assumption is defined below. Note that 

1. the equilibrium constants (dissociation constants) are in $mol~L^{-1}$, so all species must be in this unit, too! 

2. $OM$ is added to the model as a state variable removed by the slow process, and $H_xO$ is added to the model as a conserved state variable (Eq. 4c).
...
OMMpHmodel <- function(t, state, parms) {
  with (as.list(c(state, parms)), {
    ...
    H       <- solveH(K1=K1, K2=K2, Kn=Kn, Kw.prime=Kw.prime, 
                      DIC=DIC, NHx=NHx, HxO=HxO, ALK=ALK)
    HCO3    <- K1*H /  (H^2 + K1*H + K1*K2) * DIC   # eq 5a
    CO3     <- K1*K2 / (H^2 + K1*H + K1*K2) * DIC   # eq 5c
    NH4     <- H / (H + Kn) * NHx                   # eq 5e
    OH      <- Kw.prime / (Kw.prime + H) * HxO      # eq 5f, accurate
...
HxO.ini <- 55.3                # mol/L
ALK.ini <- solveALK(DIC = DIC.ini, NHx = NHx.ini, HxO = HxO.ini, pH = pH.ini)
...
We see that as a result of the mineralization of about 60 $\mu mol~L^{-1}$ of organic matter over the period of 10 days, pH decreased from 7.9 to about 7.63. As expected, $ALK$ and $H_xO$ are conserved and changes in the concentration of $H_2O$ are negligible due to vastly larger concentrations of water compared to all the other species in the system.
...
\end{array}
$$
This change, however has *no* effect on the differential equations for the lump-sum variables $DIC$, $NH_x$, $H_xO$, and $ALK$, as can easily be verified by following the same derivation steps as those leading to equations 4a--d. This can also be shown more easily: although $R_C$ adds $HCO3^-$ instead of $H_2CO_3$, these two species are both included in the $DIC$ pool, thus summing them up yields the same result for the time-derivative of $DIC$. Similarly, although the process *increases* $ALK$ at a rate $R_C$ due to the addition of $HCO3^-$, it also *decreases* $ALK$ at the same rate $R_C$ due to the addition of $H^+$, as follows from the *negative* sign in front of $[H^+]$ in the definition of $ALK$.
...
We define the initial conditions and run the model. In this step, we reuse the parameters defined for the \texttt{OMMpHmodel} model with the value of the degassing rate constant, $\lambda$, changed to $1~d^{-1}$.
...
Next, we run the \texttt{OMMpHmodel} model with the same initial conditions. By setting the OM degradation rate constant to 0, and further setting the concentrations of $DIC$ and $H_xO$ to 0, we expect to make the model equivalent to the \texttt{AmmoniaDegassing} model.
...
# initial conditions
ALK.ini    <- solveALK(DIC=0, NHx=NHx.ini, HxO=0, pH=pH.ini)
...
Thus, we expect the obtain the same results, at least for the state variables that are common for both models. This is indeed the case when we plot the results.
...
Now that we have verified that the \texttt{OMMpHmodel} can be used for solving the ammonia degassing problem, too, we can illustrate the buffering role of $DIC$ in natural waters. We will use the same initial conditions and the ammonia degassing rate constant as above and perform the calculations for $DIC$ concentrations of 0, 1, and $2~mmol~L^{-1}$.

```{r}
# without DIC (but with ammonia and water!)
ALK.ini3   <- solveALK(DIC=0, NHx=NHx.ini, HxO=HxO.ini, pH=pH.ini)
...
ALK.ini4   <- solveALK(DIC=1e-3, NHx=NHx.ini, HxO=HxO.ini, pH=pH.ini)
...
ALK.ini5   <- solveALK(DIC=2e-3, NHx=NHx.ini, HxO=HxO.ini, pH=pH.ini)
...
The results show that when DIC is present, the protons released due to ammonia degassing are scavenged by both $CO_3^{2-}$ and $HCO_3^-$ in the solution, leading to an increase in $H_2CO_3$. As a result, the increase in  proton concentrations, and thus the decrease in pH, is smaller when DIC is present, illustrating the pH buffering effect by the carbonate system. The magnitude of the effect does not, however, scale linearly with $DIC$. 
