...
\displaystyle
\frac{d[H^+]}{dt} & = & -R_1 - R_2 - R_3 - R_4 & \qquad (1g) \\[5mm]
\displaystyle
\frac{d[OH^-]}{dt} & = & -R_4 & \qquad (1h) \\[5mm]
\end{array}
$$
...
\qquad\hspace{2cm} (3b)
$$
Based on equation 1d and 2c, we obtain $R_3=0$, which yields the relationship
...
Finally, based on equation 1f and 2d, we obtain $R_4=0$, which yields the relationship
...
Here, we defined the dissociation constants $K_1$, $K_2$, $K_n$, $K_w'$ according to $K_1 = k_{1f}/k_{1b}$, $K_2 = k_{2f}/k_{2b}$, $K_n = k_{3f}/k_{3b}$, and $K_w' = k_{4f}/k_{4b}$. If these relationships are true, then the time derivatives in equations 1b--h are zero. This means that these four equations do *not* provide any *extra* information for the equilibrium concentrations.
...
Similarly, by summing equation 1f and 1h, we obtain

$$
\frac{d[H_xO]}{dt} = 0 \qquad\hspace{2cm} (4c)
$$

Finally, by summing previous equations as (1b) + $2\times$(1c) + (1h) $-$ (1d) $-$ (1g), we obtain
$$
\frac{d[ALK]}{dt} = 0 \qquad\hspace{2cm} (4c)
$$
These equations show that DIC is *added* at a rate of the *slow process* with rate $R_C$, $NH_x$ is *added* at a rate of the *slow process* $R_N$, while $H_xO$ and ALK are *conserved* (not changing over time). Specifically, *none* of the lump-sum species DIC, $NH_x$, $H_xO$, and ALK is affected by the fast reversible reactions.
...
$$
[OH^-] \approx \frac{K_w}{[H^+]} \qquad\hspace{2cm} (5f)
$$
...
\qquad\hspace{2cm} (5g)
$$
Because $[HCO_3^-]$ and $[CO_3^{2-}]$ depend on $[DIC]$ and $[H^+]$ (see 5a and 5c), $[NH_4^+]$ depends on $[NH_x]$ and $[H^+]$, and $[OH^-]$ depends on $[H^+]$, \bf equation 5g provides an intimate relationship between $[ALK]$, $[DIC]$, $[NH_x]$, and $[H^+]$ \rm when the system is in an equilibrium. Using this relationship, one can calculate any of the species provided that the other *three* species are known. Because equation 5g is highly non-linear, these calculations are done numerically.

## Task 5: Application to organic matter mineralization

### R implementation

First, we expand the function that calculates alkalinity to include ammonium as the additional input (equation 5g).

```{r}
solveALK <- function(K1=4.44e-07, K2=4.67e-11, Kn=5.68e-10, Kw=9.91e-15,
                     DIC, NHx, pH){
...
Then, we update the function that calculates $[H^+]$ from the known alkalinity, DIC and NHx concentrations (equation 5g). This is done by finding a root of a function defined as the left-hand-side minus the right-hand-side of equation 5g.

```{r}
solveH <- function(K1 = 4.44e-07, K2 = 4.67e-11, Kn = 5.68e-10, Kw = 9.91e-15,
                   DIC, NHx, ALK){
  ...
  rootFun <- function(H) {
    ...
    NH4     <- H / (H + Kn) * NHx                   # eq 5e
    OH      <- Kw / H                               # eq 5f
    ALK.est <- HCO3 + 2*CO3 + OH - NH4 - H # right-hand-side of eq. 5g
...
```{r}
(pH.new <- -log10(solveH(DIC = 2e-3, NHx = 1e-5, ALK = 2.14e-3)))
...
OMMpHmodel <- function(t, state, parms) {
  with (as.list(c(state, parms)), {
    ...
    # for output
    H <- solveH(K1 = K1, K2 = K2, Kn = Kn, Kw = Kw, DIC = DIC, NHx = NHx, ALK = ALK)
    HCO3    <- K1*H /  (H^2 + K1*H + K1*K2) * DIC   # eq 5a
    CO3     <- K1*K2 / (H^2 + K1*H + K1*K2) * DIC   # eq 5c
    NH4     <- H / (H + Kn) * NHx                   # eq 5e
    OH      <- Kw / H                               # eq 5f
