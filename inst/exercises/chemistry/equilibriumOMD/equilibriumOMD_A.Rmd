...
Thus, we expect the obtain the same results, at least for the state variables that are common for both models. This is indeed the case, as illustrated in the following graphs.

```{r, fig.width=10, fig.height=3, echo=FALSE}
out1.modif <- out2
sel.vars    <- c("NHx", "NH4", "NH3", "pH")
out1.modif[,sel.vars] <- out1[,sel.vars]
plot(out1.modif, out2, which=sel.vars,
     mfrow = c(1,4), xlab="time (d)", lwd=2, col=1:2, lty=1:2)
```
...
plot(out3, out4, out5, 
     which=c("pH", "H", "OH", "HxO", "DIC", "ALK", 
             "NHx", "NH4", "NH3", "H2CO3", "HCO3", "CO3"),
     mfrow = c(4,3), las = 1, col=1:3, lty=1:3, lwd=2)
