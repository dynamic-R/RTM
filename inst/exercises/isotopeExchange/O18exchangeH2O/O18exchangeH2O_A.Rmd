...
Finally, we implement the model function based on the differential equations 8 and 9. Values of the rate constants and the area of the liquid water will be passed via ``pars``.
...
find_state2 <- function(Vtot, Vliq1, Vliq2, humid, TK, Ptot, x18vap, x18liq1, x18liq2){
  ...
  # total amounts of vapor, liquid, and air in the gas phase:
  Nvap  <- Nwater_vapor(humid=humid, TK=TK, V=Vgas)
  Nliq1 <- Nwater_liquid(V=Vliq1)
  Nliq2 <- Nwater_liquid(V=Vliq2)
  Nair  <- Ptot * Vgas/(Rgas*TK) - Nvap
...
Water18Oexchange2 <-function(t, state, pars) {
  with (as.list(c(state, pars)),{
    ...
    # return time-derivatives and ordinary variables as a list
    list(c(dN18vap.dt, dN16vap.dt, dN18liq1.dt, dN16liq1.dt, 
...
Next, we plot pressures and the relative air humidity.
...
```
The results show that the two liquid water reservoirs reach an isotopic equilibrium after the total of about 24 hours. After this time, there is no measurable isotopic difference between the liquid reservoir 1 and 2 ($\delta_{liq1}(liq2)=0$) and the difference between the water vapor and liquid water is $-10$ \textperthousand, as required. Note that the latter difference increased from 0 to about 400 \textperthousand\ during the first 15 minutes, then decreased from 400 \textperthousand\ to about $-8$ \textperthousand\ over the following 10 hrs, and finally from $-8$ \textperthousand\ to $-10$ \textperthousand\ over the last 14 hrs. The final ${}^{18}O$ atom fraction of the water vapor and liquid reservoirs are ``r sprintf("%.4e", out3[nrow(out3),"x18vap"])`` and  ``r sprintf("%.4e", out3[nrow(out3),"x18liq1"])``, respectively.

Note that the time-scale of when the equilibrium is reached depends on the *area* of the liquid water in contact with air (the greater the area, $A_{liq}$, the shorter the time-scale), but *not* on the *volume* of the gas phase. This conclusion follows directly from the differential equations 8 and 9 and is illustrated below. Additionally, the time-scale is shorter for smaller volumes of the liquid water (not shown below).
