...
Water18Oexchange <-function(t, state, pars) {
  with (as.list(c(state, pars)),{
    ...
    x18vap           <- N18vap/Nvap
    x18liq           <- N18liq/Nliq
    R18vap           <- N18vap/N16vap
    R18liq           <- N18liq/N16liq
    delta_Vap_vs_Liq <- (R18vap/R18liq-1)*1e3
    ...
    list(c(dN18vap.dt, dN16vap.dt, dN18liq.dt, dN16liq.dt, dNair.dt),
         ...
         humid = Pvap/Peq_vapor(TK), # relative humidity
...
First, we use the above functions to evaluate the initial conditions and model parameters based on the definition of the problem. We solve the model for two initial values of the relative air humidity: 0 (scenario 1) and 1 (scenario 2). In the scenario 1, we use a very small initial humidity instead of 0 to avoid problems with division by zero.
