...
WaterEvapPrecip <-function(t, state, pars) {
  with (as.list(c(state, pars)),{
    ...
    list(c(dNvap.dt, dNliq.dt),
         ...
         # volumes
         Vgas.m3 = Vvap,      # gas phase
         Vliq.L  = Vliq*1e3,  # liquid phase
         Vtot.m3 = Vtot,      # total (gas+liquid)
         
         # pressures
         Pvap = Pvap,         # partial pressure of vapor, Pa
         Pair = Pair/101325,  # partial pressure of air, atm
         Pgas = Ptot/101325,  # total gas pressure (vapor+air), atm
...
First, we define a function that returns the values of state variables and model parameters based on the conditions defined by the problem (i.e., initial volumes and relative air humidity).

```{r}
find_state_pars <- function(Vtot, Vliq, humid, TK){
  # volume of the vapor (=gas phase)
  Vvap <- Vtot - Vliq
  ...
  SV   <- c(Nvap = Nvap, Nliq = Nliq)
  pars <- c(TK = TK, Nair = Nair)
  return(list(SV=SV, pars=pars))
}
```

Now, we can use the above function to generate the initial conditions and model parameters and use it to solve the model over the first hour.
...
outtimes <- seq(from = 0, to = 1, length.out = 100) # time in hr
out1 <- ode(y = SVini, parms = pars, func = WaterEvapPrecip, times = outtimes)
```

Subsequently, we use the end values of the first model run to calculate the initial condition for the model run over the second hour. We consider an ideal gas law when calculating the new volume of the gas phase during cooling from $T_1$ to $T_2$.

WORK in progress...

```{r}
SVini2 <- tail(out1[,c("Nvap","Nliq")],1)
...
plot(out1,
     ...
     xlab="time (hr)", lty=1, las=1, lwd=2, mfrow=c(3,4),
     ylab="mol O2")
```
