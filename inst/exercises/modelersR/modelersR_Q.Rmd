...
* ``RTM`` --- This package contains much of the teaching material for the course. This package is currently available from a Github repository ``dynamic-R/RTM`` (\url{https://github.com/dynamic-R/RTM}). You can install it by typing the following command in the R-console:
  ...
  More information about the RTM package is provided at the end of in this document (section *Learning reaction-transport modelling using the package RTM*).
...
matrices that obey certain criteria. R distinguishes logical variables *TRUE* and *FALSE*, represented by the
...
*	For a sediment model, we divide 10 cm of sediment into thin layers, 
        ...
        porosities, concentrations, etc., remain constant within 
...
defines a function (called *Circlesurface*), which takes as an input
  argument a variable called *radius*  and returns the value
...
Note that, within the function body, we ended the first line with a ``+`` in order to make clear that the statement is not finished and continues on the next line. It would have
...
(*p*), and using three different functions (*method*).
...
*	Function *setup.grid.1D* specifies a 1-Dimensional *grid*, 
  i.e., it divides a sediment column into thin layers, an estuary into $1~km$ 
...
define a function that estimates, for a certain sediment depth, $x$, the corresponding porosity. Then, we use the *ReacTran* function *setup.prop.1D* to calculate the porosity on this grid:
...
coefficients. 
  ...
  * Estimate the molecular diffusion coefficient for $O_2$ and $CO_2$, for 
  salinity = 20 and temperature = $10~^{\circ}$C. What are the units?
  Convert to $cm^2~d^{-1}$. 
  
  Note: the *diffcoeff* function from the *marelac* package returns a *data.frame* or a *list*.
  For plotting, it is easiest to subset this so as to have a vector. Thus, 
```{r, eval = FALSE}  
diffcoeff()$O2
```
...
* Estimate the molecular diffusion coefficient for $O_2$ and $CO_2$ 
  for a temperature ranging from 1 to $30~^{\circ}$C. 
  Make a temperature vs. diffusion coefficient plot for $O_2$ (in units of $cm^2~d^{-1}$).
* Add to this plot the temperature vs. diffusion coefficient relationship 
...
In order to create reaction-transport models that describe for instance 
...
* Implement the porosity profile from the previous exercise as an R-function that
  takes as input the sediment depth. 
  
* Implement the estuarine morphology from the previous exercise as an R-function. 
