...
### Important note about plot arrangements

Note that the ``deSolve`` package has its own implementation of the native R function ``plot``. A relevant detail that you need to be aware of in the context of plotting is this: when the model output generated by a function in the ``deSolve`` package is passed as input to the ``plot`` function, the graphs are *automatically* arranged based on the number of output variables. For example, the output ``out`` generated by the code above contains four output variables: ``X``, ``Y``, ``Z``, and ``sum``. Thus, ``plot(out)`` will, by default, yield four graphs arranged in a $2\times 2$ matrix, as shown in the figure above. You can change this arrangement by modifying the ``mfrow`` variable within the ``plot`` function. For example, the following code will plot the graphs as a $1\times 4$ matrix (one row, four columns):

```{r, fig.align='center', fig.height=2.3, fig.width=6}
plot(out, xlab = "time", lwd = 2,
     mfrow=c(1,4))
```

Importantly, the graph arrangement is *remembered* within a particular R-chunk. For example, the following sequence of commands will yield a graph arrangement that may not be desired:

```{r, fig.align='center', fig.height=2.3, fig.width=6}
plot(out, xlab = "time", lwd = 2, mfrow=c(1,4)) # plot 1
x <- seq(from=-2, to=2, by=0.2)
plot(x, x^2, type="l")                          # plot 2
```

If you want to use a *different* arrangement for the second plot, you need to explicitly reset it by using the ``par`` function, as shown by the following example:

```{r, fig.align='center', fig.height=2.3, fig.width=6}
plot(out, xlab = "time", lwd = 2, mfrow=c(1,4)) # plot 1
par(mfrow=c(1,1))                               # REARRANGE THE PLOTS
plot(x, x^2, type="l")                          # plot 2
```

Generally, it is recommended to execute individual ``plot`` functions separately in *different* R-chunks. This approach will not only avoid the issues described above, it will also allow you to control the positioning and size of the graph, individually for each R-chunk. The latter can be achieved by specifying values for ``fig.center``, ``fig.height`` and ``fig.width`` when starting a new R-chunk, as shown in the following example.
