...
urlcolor: blue
abstract: \noindent This short reader introduces you to the teaching and programming tools that we will use in the *Reaction-Transport Modelling* class. Sections 1 and 2 explain how to install the required programs and packages It will show you how to merge text, programming code, and the results of this code in one document. The programming language we use is R, the text is written in *markdown*, and the creation of the documents is done by *knitr*. We recommend that you recreate the examples provided in this document and then solve the exercises. In this way you will start gradually building your knowledge of R and *markdown*, which will help you understand more efficiently the material covered in the course.
---
...
First, install the *base* R (e.g., installation file ``R-4.5.2-win.exe`` or some later version). Subsequently, install *Rtools* (e.g., installation file ``rtools45-6691-6492.exe``, available under the *Installing Rtools45* section of the website describing Rtools installation). The later step will allow you to compile R-packages on your computer, which will be quite important later on. Depending on the version you are installing, you may need to read through the instructions to ensure that everything will work as intended.
  
## Rstudio

To execute R code in a nice and comprehensive environment, we will use *Rstudio*.  Go to \url{https://posit.co/downloads/}, choose *Download Rstudio*, download the installation package for your system (e.g., ``RStudio-2026.01.0-392.exe`` or some later version), and install it.
...
* ``learnr`` --- This package will allow you to view *tutorials*, which are an essential part of the course. More information about tutorials is provided later in this document (section RTM package).

* ``deSolve`` (Soetaert et al., 2010) --- This package contains methods to perform numerical integration. Thus, it allows you to solve 0-, 1- and 2-dimensional differential equations, which will be dealt with in the first part of the course.

* ``rootSolve`` (Soetaert, 2009). This package allows you to find the root of functions (steady-state).
  
* ``ReacTran`` (Soetaert and Meysman, 2012) --- This package allows you to solve differential equations describing reaction-transport models in 1D and 2D. This R-package will be used in the second part of the course. 
  
* ``marelac`` (Soetaert and Petzoldt, 2018) --- This package contains functions and constants from the aquatic sciences.

* ``AquaEnv`` (Hofmann et al. 2010) --- This package contains functions and constants for creating acid-base models.

* ``devtools`` --- This package allows you to develop R packages. We will use it to install the R package ``RTM`` from the github repository. 

* ``RTM`` --- This package contains much of the teaching material for the course. This package is currently available from a Github repository ``dynamic-R/RTM`` (\url{https://github.com/dynamic-R/RTM}). You can install it by entering the following command in the R-console:
...
We will use the typesetting program \LaTeX\ to produce beautifully typeset documents (like this one ``;-)``). This program is extremely powerful and versatile, and it is widely used especially by people who want to insert (many) equations in their documents in a convenient way. In this course we will use it to convert documents written in *markdown* into PDF.

To install \LaTeX, go to \url{https://miktex.org/}, choose *Download*, download the installation file compatible with your operating system, and install it following the installation instructions. During the installation process, select *yes* when asked *Install missing packages on-the-fly*. Also, run the *MikTeX console* from the start menu just after the installation has finished, select *check updates*, and update any packages suggested by this check. At the end, restart *Rstudio* if it was running before \LaTeX\ installation.

## Check whether everything is working 

To check whether everything is working at this point, open *Rstudio* and create a new R Markdown file. This is done from the menu by selecting 

* File $\rightarrow$ New file $\rightarrow$ R Markdown $\rightarrow$ Document.

Type the title, your name in the author box, check the ``PDF`` checkbox, and click *Ok*. If everything goes as expected, your *Rstudio* session should look similar as shown in Figure 1. 

![Screenshot of Rstudio with the default Rmd file.](images/Rstudio.png){width=98%}
...
![Nicely formatted output produced from the Rmd file. Left: PDF, viewed in SumatraPDF. Middle: DOC, viewed in Microsoft Word. Right: HTML, viewed in Firefox.](images/Rmd_pdf_html_doc.png)
...
After installing the required software, you can proceed by studying other material prepared for the reaction-transport modelling course. This material is put together in the R-package called *RTM*. You can install this package in *Rstudio* by entering the following command in the R-console:
```
devtools::install_github("dynamic-R/RTM", depend=TRUE)
```
...
The *tutorials* contain short knowledge clips about a specific modelling topic. The topics range from basic principles (e.g., conceptual diagrams, mass balance equations) to more advanced (e.g., rate laws) and very advanced topics (e.g., reaction-transport modelling in porous media, pH modelling). The tutorials also contain simple tasks and exercises, which allow you to test your knowledge gained from the videos and prepare you for the exam. 

We recommend that you study the tutorials *in advance*, i.e., *before the class*. Try to learn as much as possible by watching the knowledge clips (preferably multiple times), consult the textbook, and solve the tasks and exercises within the individual tutorials. In the process, note questions about topics that you have not fully understood. You will be able to consult these questions with your peers and with the lecturers during the class.
...
![Left: Rstudio with the Tutorial tab selected. Right: RTM tutorial *conceptual* run as a web-based app, created using the *learnr* package.](images/Rstudio_tutorial.png)

Alternatively, enter the following command in the R-console to see the complete list of available tutorials (output is shown in the table below):
```
RTMtutorial("?")
```
...
```
Note that the tutorials are sorted from basics to intermediate and advanced topics. By studying the tutorials in this order, you will be able to build up your knowledge of modelling in R in a gradual way. We will adhere to the same order during the course.

## RTM exercises

Each tutorial comes with a set of *exercises*, which contain more complicated problems that you should be able to solve once you have learned the basics from the tutorial. You will be working on these exercises *during the class* and thus be able to consult your questions, ideas, and solutions with the lecturers in a *direct and interactive way*. 

Note that the level of exercises will vary greatly as we progress with the course. We will take a slower start, but some of the exercises will get quite advanced relatively soon. It is entirely up to you to choose the pace of learning that suits you best. Note that you do *not* need to solve all the exercises. If you only want to learn the basics of modelling, it is totally fine if you only focus on the simpler exercises. However, if you do like to challenge yourself, there are couple of advanced exercises that you can try to solve. You will learn in this course as much as you are willing to dedicate to it in terms of time and effort. The lecturers will be eager to help you in the process. 
...
![Screenshot of an RTM tutorial *Reversible reactions and equilibrium chemistry*, showing the suggestions for extra exercises to solidify the knowledge gained in the tutorial.](images/RTMexercises.png)

Enter the following command in the R-console to see the complete list of available exercises (output is shown in the table below):
```
RTMexercise("?")
```
...
Finally, the *RTM* package contains *extra readers*. These readers explain topics that are not part of the course but are nevertheless useful in the context of reaction-transport modelling. For example, they explain how to visualize multi-dimensional data generated by the models, implement events and forcing functions based on data, fit data with a model, create interactive model applications, develop models collaboratively using ``git`` and the Github repository, and other topics.

### How to view a reader?

Enter the following command in the R-console to see the complete list of available readers (output is shown in the table below):
```
RTMreader("?")
```
...
```{r, eval = TRUE, fig.width = 8, fig.height = 4, fig.cap="Example of a plot generated by an R-code."}
...
![Screenshot of *Rstudio* showing an R Markdown code (left) and the resulting document produced via *knit* (right).](images/Rmarkdown_ex1.png)
...
* The R-code is embedded between the symbols ```` ``` ````\{r\} and ```` ``` ````. These are called **R-code chunks**, or **R-chunks** for short, and they can be inserted using the shortcut \texttt{Ctrl+Alt+I}. In the top-right position of each R-chunk, you will find three symbols (two of them are marked with an orange circle in Figure 6). Pressing the middle symbol will run all R-chunks above (shortcut: \texttt{Ctrl+Alt+Shift+P}), while the right symbol will run the current R-chunk (shortcut: \texttt{Ctrl+Shift+Enter}). If you move with the cursor to a specific line within the R-chunk and press \texttt{Ctrl+Enter}, the code on that line will be executed. If there is any output, it will be displayed in the R-console.

Finally, the *Knit* button (encircled in blue in Figure 6; shortcut: \texttt{Ctrl+Shift+K}) will compile the *R Markdown* document and generate the nicely typeset document (shown on the right in Figure 6). This final document contains both the R-code and the results. Thus, it provides a full documentation of the analysis.
...
In *Rstudio*, load the ``RTM`` package using the command ``require(RTM)``. Next, select the menu item File $\rightarrow$ New File $\rightarrow$ R Markdown. Then, select ``From Template`` and scroll through the list to find the ``rmarkdown_small.Rmd`` file, select it, and click ``OK`` to open it in the editor. This file was used to generate Figure 5.
...
* Generate a WORD, HTML and PDF document.
...
An R-code is easy to read once you realize that:

* ``<-`` is the assignment operator (e.g., ``A <- 1`` assigns the value of 1 to the variable A).\footnote{Note that within an R-code edited in \emph{Rstudio}, you can quickly type this operator by pressing a shortcut \textbf{Alt+minus}. This will also include spaces around the operator for increased legibility of the command. This works well for Windows and Linux users. Mac-OS users may need to search the internet for a suitable shortcut.}
...
There are three ways to work with R, as described next.
...
In the console window, the ``up`` and ``down`` arrow keys can be used to navigate through previously typed commands (i.e., the command history). Note that typing commands in the console is not the most efficient way of using R, especially if you want to debug and save your work for later use (see next).
...
If you have a specific problem, just type *R: your problem* on your search engine. Chances are that someone encountered the problem and it was already solved.
...
will give a list of available demonstrations in the main software, whereas
  
```{R, eval = FALSE }
 demo(transport1D)
```
  
will demonstrate some modelling output generated with the package *ReacTran*.
...
* $log_2(4096)$
* $2\times \pi\times 3$
* $\sqrt {2.3^2  + 5.4^2  - 2\times 2.3\times 5.4\times \cos (\pi /8)}$
...
1/0           # yields Inf
0/0           # yields NaN
2.3e-8 * 1000 # it is recommended to use 2.3e-8 rather than 2.3*10^-8
```
...
Note that the ``data`` are input as a vector (using the ``c()`` function), and these values are sorted into the matrix column after column (by default). The following commands display the matrix and the square root of its elements.
...
*	specify a vector of logical values (TRUE/FALSE) to indicate which elements to include (TRUE) and which not to include (FALSE). This option uses logical expressions.
...
Logical expressions are often used to select elements from vectors and matrices that obey certain criteria. R distinguishes logical variables ``TRUE`` and ``FALSE``, represented by the integers $1$ and $0$.\footnote{Note that in \textit{R}, variables \texttt{T} and \texttt{F} may be reserved to represent the logical \texttt{TRUE} and \texttt{FALSE}, respectively. Therefore, if you want to avoid unexpected behavior, you should \textit{not} use \texttt{T} and \texttt{F} as variables to which you assign values. For example, you might be tempted to use \texttt{T <- 20} for setting the temperature value to 20, or \texttt{F <- 0.1} for setting the flux value to 0.1. It is recommended that you do \textit{not} do this, but use instead more intuitive names such as \texttt{Temp} and \texttt{Flux}.}
...
will set to zero all elements in V that are greater or equal to 1.

## Removing specific elements

When the index is preceded by a ``-`` (minus) sign, the element is removed. For example,

```{r}
 A[ ,-1]
```

will select the content of the matrix ``A`` except for the first column, while the command

```{r}
A[-2, ]
```

will select the content of the matrix ``A`` except for the second row.
...
*	Use the R-function ``mean`` to estimate the mean of two numbers, 9 and 17. (You may notice that this is not as simple as you might think! Tip: use the ``c()`` function.)

### Sediment depth profiles 
*	For a sediment model, we divide 10 cm of sediment into thin layers, each 1mm thick. These are thin slices, because it is assumed that porosities, concentrations, etc., remain constant within each layer. We need to know the depth in the centre of each sediment layer. 
To do so, we need to create a sequence of depth values, extending from 0.05 cm to 9.95 cm, at 1 mm intervals.
    - Create this sequence, put it in a vector called ``depth``. Use the R-function  ``seq``. 
    - Display this vector.
...
$$porosity = 0.7+(1-0.7)\times e^{-depth}$$
...
* The estuary is 100 km long, and the cross-sectional surface, $A$ (in $m^2$), as a function of the distance, $x$ (in $m$), can be approximated with the following expression: $$A(x) = A_r + dA \times  \frac{x^p}{x^p + {k_s}^p}$$ where $dA = A_s - A_r$, p = $5$, ks = $50000~m$, $A_r = 4000~m^2$, $A_s = 76000~m^2$. Here $A_r$ and  $A_s$ are the cross-sectional surfaces at the boundary with the river and the 
sea, respectively. 
...
*	Function ``setup.grid.1D`` specifies a 1-Dimensional *grid*, i.e., it divides a sediment column into thin layers, an estuary into $1~km$ thick boxes, a ciliate organism into $1~\mu m$ thick concentric spheres, etc.
...
*	Estimate the saturated oxygen concentration for a range of temperatures from 0 to $30~^{\circ}$C, and salinity 35. 
...
* Estimate the molecular diffusion coefficient for $O_2$ and $CO_2$, for salinity = 20 and temperature = $10~^{\circ}$C. What are the units? Convert to $cm^2~d^{-1}$. 
...
* Estimate the molecular diffusion coefficient for $O_2$ and $CO_2$ for a temperature ranging from 1 to $30~^{\circ}$C. Make a temperature vs. diffusion coefficient plot for $O_2$ (in units of $cm^2~d^{-1}$).
* Add to this plot the temperature vs. diffusion coefficient relationship for $O_2$ at salinity = 0. Use the R function ``lines`` or ``points`` to add these data.
...
* What is the area (in mm$^2$) of each of these concentric spheres? Implement as an R-function.

### Porosity profile and estuarine morphology as a function

* Implement the porosity profile from the previous exercise as an R-function that  takes as input the sediment depth. 
  
* Implement the estuarine morphology from the previous exercise as an R-function.   Return both the cross-section area and volume for each box.
...
Here, $A$ and $B$ are called the *state variables*, $\frac{dA}{dt}$ and $\frac{dB}{dt}$ are the *time derivatives* (also called the *rates of change*), while $r$, $x$, $y$ and $k$ are constant parameters.
...
* give values to the parameters (``pars``);
*	assign initial conditions to the state variables (``state.ini``);
*	generate a sequence of time values at which we want the output  (``time.seq``).
...
*	Write an R-script that solves the Lotka-Volterra model
$$ \frac{{dX}}{{dt}} = a \cdot X \cdot \left(1 - \frac{X}{k}\right) - b \cdot X \cdot Y$$
$$     \frac{{dY}}{{dt}} = g \cdot b \cdot X \cdot Y - e \cdot Y$$
for the initial values of $X = 670$ and $Y = 610$, and for the parameter values of $a = 0.04$, $k = 1000$, $b = 5\times 10^{-5}$, $g = 0.8$, and $e = 0.008$.

*	Run the model for 100 days, and store the output in a variable ``out``.
* Plot the outcome.
*	Now run the model with other initial values ($X = 100$, $Y = 540$); store the output in the variable ``out2``. Plot the two model runs simultaneously.
* Experiment by running the model for longer intervals (e.g., 1500 days) and by changing the model parameter ``b`` in a range between $1\times 10^{-5}$ and $10\times 10^{-5}$. What do these model predictions tell you about the system?
...
* It takes about 10 lines of an R-code to generate the solutions and plot them. Try it out.
* Use the initial conditions $X = Y = Z = 1$.
* Create output for a time sequence ranging from 0 to 100, and with a time step of 0.005.
