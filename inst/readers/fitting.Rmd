...
vignette: >
  %\VignetteIndexEntry{Fitting a 1D Reaction Transport Model to Data in R: accessory material}
...
```{r}
# model function
PDiamodel <- function (t, state, pars)   
{
  with (as.list(pars),{
...
std <- steady.1D (y=ini, func=PDiamodel, parms=pD, 
...
Suppose that we have the following experimental data for the concentrations of DIC and PO4 (mol/m3) at several depths (m) of the sediment:
...
std.fun <- function(pars){
  std <- steady.1D (y=ini, func=PDiamodel, parms=pars, nspec=nspec, dimens=N, 
...
Original <- steady.1D(y=ini, func=PDiamodel, parms=default.parms, 
                   nspec=nspec, dimens=N, names=names,
                   positive=TRUE) 
BestFit  <- steady.1D(y=ini, func=PDiamodel, parms=p, 
                   nspec=nspec, dimens=N, names=names,
                   positive=TRUE) 
...
std2 <- steady.1D(y=ini, func=PDiamodel, parms=pD2, 
                  nspec=nspec, dimens=N, names=names,
                  pos=TRUE)     
...
std.fun2 <- function(pars){
  std <- steady.1D (y=ini, func=PDiamodel, parms=pars, nspec=nspec, 
