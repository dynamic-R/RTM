...
Server.Si <- shinyServer(function(input, output, session) {
  ...
  observeEvent(input$resetButton, {
    updateNumericInput(session, "NV",    value = default.pars$NV)
    updateNumericInput(session, "kp",    value = default.pars$kp)
    updateNumericInput(session, "Ceq",   value = default.pars$Ceq)
    updateNumericInput(session, "Rini",  value = default.pars$Rini)
    updateNumericInput(session, "Cini",  value = default.pars$Cini)
  })
 ...
 # Get the model parameters, as defined in the UI 
  ...
  output$PlotSi <- renderPlot({     # will be visible in the main panel
   ...
   pars      <- getpars()
   state.ini <- InitialCondition.fun(pars)
