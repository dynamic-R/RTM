...
$$
Obviously, this numerical approximation will be more accurate for smaller values of $\Delta x$.
...
$$
\frac{\partial S}{\partial t} = -\frac{\partial}{\partial x}\left( v \cdot S - D \cdot \frac{\partial S}{\partial x}\right),
$$
...
From this vector, the advective+diffusive flux at the boundary between cell $i-1$ and $i$ is estimated according to:
$$
...
The temporal derivative is then estimated based on these fluxes according to: 
$$
...
$$
where $f'(y_0)$ is the derivative of $f$, evaluated at $y_0$.
...
The Jacobian matrix itself can be created by numerical approximation: $$\frac{\partial f(y_i)}{\partial y_j} = \frac{f(y^*)_i-f(y)_i}{\Delta y_j},$$ where $y^*$ is the y-vector that contains the perturbed value of $y_j$, i.e., $$y^*=y+[0, \ldots, \Delta y_j, \ldots, 0].$$
...
A lot of time can be saved if the Jacobian matrix is efficiently created and inverted (or decomposed). We will deal with how this is used in rootSolve later.
...
Euler integration essentially finds the solution for the next time step, $y_{t+\Delta t}$, by combining the values at the previous time step, $y_t$, with the derivative at the previous time step, $({dy/dt})_t$, as follows:
$$
y_{t+\Delta t} = y_t + \Delta t \cdot \left(\frac{dy}{dt}\right)_t
$$
...
$$
\left(\frac{dy}{dt}\right)_t \approx \frac{y_{t+\Delta t} - y_t}{ \Delta t},
$$
which is, in turn, derived from the mathematical definition of the derivative,
$$
\left(\frac{dy}{dt}\right)_t = \lim_{\Delta t \rightarrow 0}  \frac{y_{t+\Delta t} - y_t}{ \Delta t},
$$
as explained at the beginning of this document.
...
$$
y_{t+\Delta t} = y_t + \Delta t \cdot \left(\frac{dy}{dt}\right)_{t+\Delta t}
$$
Note that the derivative $\frac{dy}{dt}|_{t+\Delta t}$ is estimated at $t+\Delta t$, i.e., using the unknown value of $y_{t+\Delta t}$.
...
$$
g(y_{t+\Delta t}) = y_{t+\Delta t} - y_t - \Delta t \cdot \left(\frac{dy}{dt}\right)_{t+\Delta t}
$$
